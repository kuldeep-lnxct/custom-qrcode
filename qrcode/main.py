...
class QRCode(Generic[GenericImage]):
    ...
    def get_point_type(self, row: int, col: int) -> int:
        """
        Return the semantic point type for the module at (row, col).
        Used by qrbtf-style drawers to render finder, alignment, timing, and
        data modules differently. Returns QRPointType enum value.
        """
        if self._type_table is None:
            self._type_table = get_type_table(self.modules_count)
        if not (0 <= row < self.modules_count and 0 <= col < self.modules_count):
            return int(QRPointType.DATA)
        return self._type_table[row][col]
